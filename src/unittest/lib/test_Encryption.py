...
class Test(unittest.TestCase):
    ...
    def test_get_aes(self):
        ...
        self.assertIsInstance(self.enc.get_aes(IV), Cipher._mode_cbc.CbcMode)
    ...
    def test_set_salt(self):
        salt = self.enc.gen_salt()
        self.enc.set_salt(salt)
        self.assertEqual(self.enc.salted_key, salt + self.key)

    def test_encrypt(self):
        secret_string = b'my secret string'
        encrypted = self.enc.encrypt(secret_string)
    ...
    def test_decrypt(self):
        secret_string = b'my secret string'
        encrypted = self.enc.encrypt(secret_string)
        self.assertEqual(self.enc.decrypt(encrypted), secret_string)
    ...
    def test_decrypt_3(self):
        ...
        self.enc.set_salt(salt)
        encrypted = self.enc.encrypt(secret_string)
        self.assertRaises(ValueError, self.enc.decrypt, encrypted)
